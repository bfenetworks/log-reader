...
验证 `bfe-pblog-tool tail` 在指定不存在的文件路径时能正确报错并返回非零退出码。
...
| CLI 命令 | `bfe-pblog-tool tail /nonexistent/path/to/file.log` |

## 操作步骤

1. 编译 `bfe-pblog-tool` 二进制。
2. 调用 `RunPblogTool("tail", "/nonexistent/path/to/file.log")` 执行 tail 命令。
3. 检查退出码和错误信息。

## 预期结果

- 退出码非零。
- `err` 非 nil，或 stderr 包含错误信息。
