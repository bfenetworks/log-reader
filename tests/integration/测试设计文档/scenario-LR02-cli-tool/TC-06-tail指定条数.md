...
验证 `bfe-pblog-tool tail -n 3` 能正确输出最后 3 条记录。

## 运行模式

命令行模式：测试代码通过 `common.ProcessEnv.RunPblogTool` 调用 `bfe-pblog-tool tail -n 3` 二进制。
...
| CLI 命令 | `bfe-pblog-tool tail -n 3 <logfile>` |

## 操作步骤

1. 编译 `bfe-pblog-tool` 二进制。
2. 通过 `LogGenerator` 写入 9 条 b2log 记录。
3. 调用 `RunPblogTool("tail", "-n", "3", logFile)` 执行 tail -n 3 命令。
4. 检查退出码和输出记录数。
